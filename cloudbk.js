...
if (cloudButtonDiv) {
    ...
    <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 448 512" class="absolute bottom-1 right-1 w-2 h-2 shadow text-yellow-500" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
    ...
    newButton.addEventListener('click', function () {
        ...
        pluginSwitch.addEventListener('click', function () {
            ...
            console.log('Plugin switch:', isChecked ? 'ON' : 'OFF'); 
            if (isChecked) {
                ...
                // Enable the cloud backup button and text inputs
                cloudImportBtn.removeAttribute('disabled');
                cloudExportBtn.removeAttribute('disabled');
                dbApiKeyInput.removeAttribute('disabled'); 
                dbAppIdInput.removeAttribute('disabled'); 
...
async function exportToCloud() {
    ...
    console.log(`App ID: ${appId}, API Key: ${apiKey}, Database: ${dbName}, Collection: ${collectionName}, Document ID: ${docId}`); // Log the data

    if (!appId || !apiKey || !dbName || !collectionName) {
        alert('Please fill out the MongoDB App ID, API Key, Database Name, and Collection Name.');
        return;
    }
...
async function importFromCloud() {
    ...
    if (!appId || !apiKey || !dbName || !collectionName || !docId) { 
        alert('Please fill out the MongoDB App ID, API Key, Database Name, Collection Name, and Document ID.');
        return;
    }
